...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static void CreateRandomMonster(out Monster monster)
        {
            ...
            int randMonster = rand.Next(1, 4); //랜덤 숫자 받기
            switch (randMonster)
            {
        ...
        static void EnterFeild(ref Player player)
        {
            while (true)
            {
                ...
                else if (input == "2") // 33% 확률로 도망
                {
                    Random rand = new Random();
                    int randValue = rand.Next(0, 101); //랜덤 값 받기
                    if (randValue <= 33) //그 중 3분의 1
                    {
